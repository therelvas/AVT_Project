...
void Surface::setupObjects() {
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(surfaceTexCoords), surfaceTexCoords, GL_STATIC_DRAW);
	glEnableVertexAttribArray(VSShaderLib::TEXTURE_COORD_ATTRIB);
	glVertexAttribPointer(VSShaderLib::TEXTURE_COORD_ATTRIB, 2, GL_FLOAT, 0, 0, 0);

	//index buffer
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, buffers[3]);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(surfaceFaceIndex), surfaceFaceIndex, GL_STATIC_DRAW);
...
void Surface::setColor(VSResourceLib::MaterialSemantics mSemantics, float *values) {
...
void Surface::setMaterial(VSShaderLib shader) {
...
unsigned int Surface::loadRGBATexture(std::string filename,
...
void Surface::addTexture(unsigned int unit, std::string filename) {
...
void Surface::render(VSShaderLib shader) {
